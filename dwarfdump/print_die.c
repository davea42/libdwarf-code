    ...
    for (;;) {
        ...
        if (glflags.gf_check_abbreviations &&
            ...
            if (abtres == DW_DLV_OK) {
                ...
                glflags.gf_count_major_errors++;
                printf("ERROR: Unable to read die children flag.\n");
...
print_location_description(Dwarf_Debug dbg,
    ...
    Dwarf_Error *err)
{
    /*  The attribute is a location description or location list. */
    int res = 0;
    Dwarf_Half theform = 0;
    Dwarf_Half directform = 0;
    ...
    Dwarf_Error *err)
{
    ...
    case DW_AT_producer:
        ...
        if (attr == DW_AT_producer &&
            (glflags.gf_do_check_dwarf || glflags.gf_search_is_on)) {
