...
typedef Dwarf_Bool(*_dwarf_iterate_frame_callback_function_type)(Dwarf_Frame table, 
    Dwarf_Addr subsequent_pc, Dwarf_Bool is_last_row, void* user_data);

int
_dwarf_iterate_frame_instr(Dwarf_Fde fde,
    _dwarf_iterate_frame_callback_function_type callback,
...
#define SERINST(m)                    \
    ...
    Dwarf_Addr last_callback_address = -1;
    ...
    struct Dwarf_Frame_s local_table;
    memset(&local_table, 0, sizeof(struct Dwarf_Frame_s));

    local_table.fr_reg = localregtab;
    local_table.fr_reg_count = reg_count;

    local_table.fr_loc = initial_loc; /* code location/
        pc-value corresponding to the frame instructions.
        Starts at zero when the caller has no value to pass in. */

    struct Dwarf_Reg_Rule_s* cfa_reg = &local_table.fr_cfa_rule;
    ...
    while ((instr_ptr < final_instr_ptr) && (!search_over)) {
        ...
        case DW_CFA_advance_loc: {
            ...
            possible_subsequent_pc = local_table.fr_loc +
                (Dwarf_Unsigned)adv_pc;
            if (possible_subsequent_pc < local_table.fr_loc &&
        	...
        	/* Invoke callback only if the location actually changed */
        	if (possible_subsequent_pc > local_table.fr_loc) {
				search_over = callback(&local_table, possible_subsequent_pc, 
					FALSE, callback_user_data);
				last_callback_address = local_table.fr_loc;
			}

            /* If gone past pc needed, retain old pc.  */
            if (!search_over) {
                local_table.fr_loc = possible_subsequent_pc;
            }
            if (make_instr) {
                dfi->fi_fields = "uc";
        ...
        case DW_CFA_set_loc: {
            ...
            instr_ptr += address_size;
            if (new_loc != 0 && local_table.fr_loc != 0) {
        ...
        case DW_CFA_advance_loc1:
        {
            ...
            /* CHECK OVERFLOW add */
            possible_subsequent_pc = local_table.fr_loc + adv_loc;
            if (possible_subsequent_pc < local_table.fr_loc &&
    ...
    /*  If we have not raised a callback for the current address yet,
        do so now if the address is still within the fde. */
	if (local_table.fr_loc != last_callback_address && 
        local_table.fr_loc >= fde->fd_initial_location &&
        local_table.fr_loc < fde->fd_initial_location + fde->fd_address_range) {

        callback(&local_table, possible_subsequent_pc, TRUE, callback_user_data);
    }
...
struct Dwarf_Iterate_FDE_Info {
    Dwarf_Regtable3* reg_table;
    dwarf_iterate_fde_info_callback_function_type callback;
    void* callback_user_data;
};

Dwarf_Bool _dwarf_iterate_fde_info_for_all_regs3_callback(Dwarf_Frame table,
	Dwarf_Addr subsequent_pc, Dwarf_Bool is_last_row, void* user_data)
{
	struct Dwarf_Iterate_FDE_Info* data =
		(struct Dwarf_Iterate_FDE_Info*)user_data;

    Dwarf_Regtable3* reg_table = data->reg_table;

    {
        ...
        Dwarf_Unsigned j = 0;
        Dwarf_Regtable_Entry3* targ = reg_table->rt3_rules;
        struct Dwarf_Reg_Rule_s* src = table->fr_reg;
        for (; j < reg_table->rt3_reg_table_size;
            ++j, targ++, src++) {
            targ->dw_offset_relevant = src->ru_is_offset;
            targ->dw_args_size = src->ru_args_size;
            targ->dw_value_type = src->ru_value_type;
            targ->dw_regnum = (Dwarf_Half)src->ru_register;
            targ->dw_offset = (Dwarf_Unsigned)src->ru_offset;
            targ->dw_block = src->ru_block;
        }
    }

    reg_table->rt3_cfa_rule.dw_offset_relevant =
        table->fr_cfa_rule.ru_is_offset;
    reg_table->rt3_cfa_rule.dw_value_type =
        table->fr_cfa_rule.ru_value_type;
    reg_table->rt3_cfa_rule.dw_regnum =
        (Dwarf_Half)table->fr_cfa_rule.ru_register;
    reg_table->rt3_cfa_rule.dw_offset =
        (Dwarf_Unsigned)table->fr_cfa_rule.ru_offset;
    reg_table->rt3_cfa_rule.dw_block =
        table->fr_cfa_rule.ru_block;
    reg_table->rt3_cfa_rule.dw_args_size =
        table->fr_cfa_rule.ru_args_size;

    data->callback(table->fr_loc, reg_table, data->callback_user_data);

    return FALSE;
}

int
dwarf_iterate_fde_info_for_all_regs3(Dwarf_Fde fde,
    Dwarf_Half      reg_table_size,
	dwarf_iterate_fde_info_callback_function_type callback,
	void*           callback_user_data,
	Dwarf_Error*    error)
{
    ...
    reg_table.rt3_reg_table_size = reg_table_size;
	reg_table.rt3_rules = calloc(reg_table.rt3_reg_table_size,
		sizeof(struct Dwarf_Regtable_Entry3_s));

	if (!reg_table.rt3_rules) {		
		_dwarf_error_string(dbg, error,
			DW_DLE_ALLOC_FAIL,
			"Failure allocating Dwarf_Regtable_Entry3_s "
			"in dwarf_iterate_fde_info_for_all_regs3()");
		return DW_DLV_ERROR;
	}

	{
        ...
        user_data.callback_user_data = callback_user_data;
        user_data.reg_table = &reg_table;

        res = _dwarf_iterate_frame_instr(fde,
            &_dwarf_iterate_fde_info_for_all_regs3_callback,
            &user_data,
            /* make_instr= */ FALSE,
            fde->fd_initial_location,
            fde->fd_fde_instr_start,
