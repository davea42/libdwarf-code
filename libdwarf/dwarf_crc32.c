/* crc32.c
   ...
   combinations without any restriction coming from the use
   of this file.  (The General Public License restrictions
   do apply in other respects; for example, they cover
   modification of the file, and distribution when not linked
...
*/
/*  Like nearly all of libdwarf the license intended
    here is LGPL V2.1. a license compatible with the GPL.
...
/*  function name altered to fit into libdwarf naming. 

    dwarf_basic_crc32() is exactly the interface
    defined above.  It might be useful to some,
    but at this time libdwarf only calls it
    from dwarf_crc32().

    dwarf_crc32 is an interface indented for calling
    from code having a Dwarf_Debug.
*/
